...
def get_ind_file(filetype, weighting="vw", n_inds=30):
    ...
    ind = pd.read_csv(f"data/ind{n_inds}_m_{name}.csv", header=0, index_col=0, 
...
def get_ind_nfirms(n_inds=30):
    """
    Load and format the Ken French 30 Industry Portfolios Average number of 
        Firms
    """
...
def get_ind_size(n_inds=30):
    """
    Load and format the Ken French 30 Industry Portfolios Average size (market 
        cap)
    """
...
def semideviation(r):
    ...
    """
    excess = r - r.mean()
    excess_neg = excess[excess < 0]
    return excess_neg.std(ddof = 0)
...
def var_historic(r, level = 5):
    ...
    Returns the historic Value at Risk at a specified level i.e. returns the 
        number such that "level" percent of the returns fall below that number, 
...
def gmv(cov):
    """
    Returns the weights of the Global Minimum Vol portfolio given the 
        covariance matrix
    """
    n = cov.shape[0]
...
def plot_ef(n_points, er, cov, 
    ...
    if show_cml:
        ...
        r_mst = portfolio_return(w_msr, er)
        vol_msr = portfolio_vol(w_msr, cov)
        cml_x = [0, vol_msr]
        cml_y = [riskfree_rate, r_mst]
        ax.set_xlim(left = 0)
...
def gbm(n_years = 10, n_scenarios = 1000, mu = 0.07, sigma = 0.15, 
    ...
    rets_plus_1[0] = 1
    ret_val = (s_0 * pd.DataFrame(rets_plus_1).cumprod() 
              if prices else rets_plus_1-1)
    return ret_val

def discount(t, r):
    """
    Compute the price of a pure discount bond that pays a dollar at time period 
        t and r is the per-period interest rate returns a |t| x |r| Series or 
        DataFrame r can be a float, Series or DataFrame returns a DataFrame 
        indexed by t
    """
...
def pv(flows, r):
    """
    Compute the present value of a sequence of cash flows given by the time (as 
        an index) and amounts r can be a scalar, or a Series or DataFrame with 
        the number of rows matching the num of rows in flows
    """
    dates = flows.index
...
def bond_cash_flows(maturity, principal=100, 
                    coupon_rate=0.03, coupons_per_year=12
                   ):
    """
...
def bond_price(maturity, principal=100, coupon_rate=0.03, 
    ...
    If discount_rate is a DataFrame, then this is assumed to be the rate on 
        each coupon date and the bond value is computed over time.
        i.e. The index of the discount_rate DataFrame is assumed to be the 
        coupon number
    """
...
def bond_total_return(monthly_prices, principal, coupon_rate, coupons_per_year):
    ...
    Assumes that dividends (coupons) are paid out at the end of the period 
        (e.g. end of 3 months for quarterly div) and that dividends are 
        reinvested in the bond
    """
    coupons = pd.DataFrame(data = 0.0, # Initialize coupons as float
...
def cir(n_years = 10, n_scenarios=1, a=0.05, b=0.03, sigma=0.05, steps_per_year=12, r_0=None):
    ...
    shock = np.random.normal(0, scale=np.sqrt(dt), 
...
def macaulay_duration(flows, discount_rate):
    ...
    return np.average(flows.index, weights = weights[0])

def match_durations(cf_t, cf_s, cpy_s, cf_l, cpy_l, discount_rate):
    """
    Returns the weight W in cf_s that, along with (1-W) in cf_l will have an 
        effective duration that matches cf_t
    """
    d_t = macaulay_duration(cf_t, discount_rate)
    d_s = macaulay_duration(cf_s, discount_rate / cpy_s) / cpy_s
    d_l = macaulay_duration(cf_l, discount_rate / cpy_l) / cpy_l
    return (d_l - d_t)/(d_l - d_s)

def bt_mix(r1, r2, allocator, **kwargs):
    """
    Runs a back test (simulation) of allocating between a two sets of returns
        r1 and r2 are T x N DataFrames or returns where T is the time step index
        and N is the number of scenarios. 
    allocator is a function that takes two sets of returns and allocator 
        specific parameters, and produces an allocation to the first portfolio 
        (the rest of the money is invested in the GHP) as a T x 1 DataFrame 
...
def terminal_stats(rets, floor = 0.8, cap = np.inf, name = "Stats"):
    ...
    rets is a T x N DataFrame of returns, where T is the time-step (we assume 
        rets is sorted by time)
    ...
    e_short = ((floor-terminal_wealth[breach]).mean() 
               if breach.sum() > 0 else np.nan)
    e_surplus = ((-cap+terminal_wealth[reach]).mean() 
                 if reach.sum() > 0 else np.nan)
...
def glidepath_allocator(r1, r2, start_glide = 1, end_glide = 0):
    ...
    n_points = r1.shape[0]
    n_col = r1.shape[1]
    path = pd.Series(data = np.linspace(start_glide, end_glide, 
                                        num = n_points))
...
def floor_allocator(psp_r, ghp_r, floor, zc_prices, m=3):
    ...
    Returns a DataFrame with the same shape as the psp/ghp representing the 
        weights in the PSP
    """
    ...
    for step in range(n_steps):
        ...
        # recompute the new account value at the end of this step
        account_value = (psp_alloc*(1+psp_r.iloc[step]) 
...
def regress(dependent_variable, explanatory_variables, alpha=True):
    """
    Runs a linear regression to decompose the dependent variable into the 
        explanatory variables
    returns an object of type statsmodel's RegressionResults on which you can 
        call
...
def style_analysis(dependent_variable, explanatory_variables):
    """
    Returns the optimal weights that minimizes the Tracking error between a 
...
def portfolio_tracking_error(weights, ref_r, bb_r):
    """
    returns the tracking error between the reference returns and a portfolio of 
        building block returns held with given weights
    """
    return tracking_error(ref_r, (weights*bb_r).sum(axis=1))

def weight_ew(r, cap_weights=None, 
              max_cw_mult=None, 
              microcap_threshold=None, 
              **kwargs):
    """
    Returns the weights of the EW portfolio based on the asset returns "r" as a 
        DataFrame
    If supplied a set of capweights and a capweight tether, it is applied and 
        reweighted 
    """
    n = len(r.columns)
    ...
    if cap_weights is not None:
        ...
        # unclear why the above was in the sample code

        # cap weights at the rebalance date (end of window)
        # cw = cap_weights.loc[r.index[-1]]  
        ## exclude microcaps
...
def weight_cw(r, cap_weights, **kwargs):
    """
    Returns the weights of the CW portfolio based on the time series of 
        capweights
    """
    # unclear why the sample code used weights from the second day of the window
    w = cap_weights.loc[r.index[1]]
    # w = cap_weights.loc[r.index[-1]]
    return w/w.sum()

def backtest_ws(r, estimation_window=60, 
                weighting=weight_ew, 
                verbose=False, 
                **kwargs):
    """
    ...
    weights = pd.DataFrame(weights, index=r.iloc[estimation_window:].index, 
                           columns=r.columns)
...
def weight_gmv(r, cov_estimator=sample_cov, **kwargs):
    """
    Produces the weights of the GMV portfolio given a covariance matrix of the 
        returns 
    """
...
def cc_cov(r, **kwargs):
    """
    Estimates a covariance matrix by using the Elton/Gruber Constant Correlation
        model
    """
    rhos = r.corr()
    n = rhos.shape[0]
    ...
    sd = r.std()
    return pd.DataFrame(ccor * np.outer(sd, sd), 
                        index=r.columns, columns=r.columns)

def shrinkage_cov(r, delta=0.5, **kwargs):
    """
    Covariance estimator that shrinks between the Sample Covariance and the 
...
def risk_contribution(w,cov):
    """
    Compute the contributions to risk of the constituents of a portfolio, given 
...
def weight_erc(r, cov_estimator=sample_cov, **kwargs):
    """
    Produces the weights of the ERC portfolio given a covariance matrix of the 
        returns 
    """
    est_cov = cov_estimator(r, **kwargs)
